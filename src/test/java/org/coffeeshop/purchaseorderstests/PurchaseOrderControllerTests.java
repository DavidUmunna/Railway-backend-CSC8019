 ...
 * on the purchase order endpoints. Tests are grouped by operation using @Nested classes
...
class PurchaseOrderControllerTests {
    ...
    @Autowired
    private OrderAutoCancellation orderAutoCancellation;

    /**
     * Tests for POST /api/v1/orders — order creation scenarios
     * including happy path, validation errors, missing entities, and edge cases.
     */
    @Nested
    class CreateOrderTests {
        /**
         * Verifies that creating an order with valid data returns 201 with the correct
         * order fields including orderId, customerId, stationId, orderStatus (ACCEPTED),
         * and the calculated totalAmount. Also asserts the order is persisted in the repository.
         *
        ...
        void createOrder_returnsCreatedOrder() throws Exception {
            ...
            int quantity = 2;
            CreatePurchaseOrderDto request = new CreatePurchaseOrderDto(
                customer.getId(),
                station.getId(),
                LocalTime.of(12, 30),
                List.of(new CreateOrderItemDto(menuItemType.getMenuItemTypeId(), quantity))
            );
        ...
        /**
         * Verifies that creating an order with quantity 0 returns 400 Bad Request,
         * as the @Positive constraint on CreateOrderItemDto.quantity rejects non-positive values.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test
        ...
        /**
         * Verifies that creating an order with a non-existent customerId returns 404 Not Found,
         * as the service throws EntityNotFoundException when the customer cannot be found.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test
        ...
        /**
         * Verifies that creating an order with a null customerId returns 400 Bad Request,
         * as the @NotNull constraint on CreatePurchaseOrderDto.customerId rejects null values.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test
        ...
        /**
         * Verifies that creating an order with a non-existent stationId returns 404 Not Found,
         * as the service throws EntityNotFoundException when the station cannot be found.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test
        ...
        /**
         * Verifies that creating an order with an empty orderItems list returns 400 Bad Request,
         * as the @NotEmpty constraint on CreatePurchaseOrderDto.orderItems rejects empty lists.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test
         ...
         * returns 400 Bad Request, as the service validates the pickup time against the station schedule.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test
        ...
        /**
         * Verifies that creating an order with multiple order items returns 201 Created
         * and that the totalAmount is correctly calculated as the sum of (unitPrice * quantity)
         * for each item.
         *
         * @throws Exception if the MockMvc request fails
         */
        ...
        void createOrder_returns201ForMultipleItems() throws Exception {
            ...
            CreatePurchaseOrderDto request = new CreatePurchaseOrderDto(
                    ...
                    station.getId(),
                    LocalTime.of(12,30),
                    List.of(new CreateOrderItemDto(regLatte.getMenuItemTypeId(), latteQuantity), new CreateOrderItemDto(regAmericano.getMenuItemTypeId(), americanoQuantity))
            );
    ...
    /**
     * Tests for GET /api/v1/orders — order retrieval scenarios
     * including get by ID, get all, filter by status, and filter by customer.
     */
    @Nested
    class GetOrderTests {
        /**
         * Verifies that retrieving an existing order by ID returns 200 OK
         * with the correct orderId, customerId, stationId, and orderStatus.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test
        void getOrderById_returnsOrder() throws Exception {
            ...
            CreatePurchaseOrderDto createRequest = new CreatePurchaseOrderDto(
                    customer.getId(),
                    station.getId(),
                    LocalTime.of(12, 30),
        ...
        */
        ...
        void getOrder_returnsListOfAllOrders() throws Exception {
            ...
            CreatePurchaseOrderDto orderOne = new CreatePurchaseOrderDto(
                    customer.getId(),
                    station.getId(),
                    LocalTime.of(12, 30),
                    List.of(new CreateOrderItemDto(menuItemType.getMenuItemTypeId(), 2))
            );
            CreatePurchaseOrderDto orderTwo = new CreatePurchaseOrderDto(
                    customer.getId(),
                    station.getId(),
                    LocalTime.of(10, 30),
                    List.of(new CreateOrderItemDto(menuItemType.getMenuItemTypeId(), 1))
        ...
        /**
         * Verifies that filtering orders by a status with no matching orders
         * returns 200 OK with an empty list.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test 
        ...
        /**
         * Verifies that filtering orders by status returns 200 OK
         * with a list containing only orders matching the given status.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test 
        ...
        /**
         * Verifies that filtering orders by customerId returns 200 OK
         * with a list containing only orders belonging to the specified customer.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test 
        ...
        /**
         * Verifies that filtering orders by a non-existent customerId returns 200 OK
         * with an empty list.
         *
         * @throws Exception if the MockMvc request fails
         */
        ...
        /**
         * Verifies that filtering orders by staffId returns 200 OK
         * with a list containing only orders belonging to the specified staff.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test 
        void getOrdersByStaff_returnsFilteredList() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/orders?staffId={staffId}", staff.getStaffId()))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.length()").value(1))
                    .andExpect(jsonPath("$[0].staffId").value(staff.getStaffId()));
        }

        /**
         * Verifies that filtering orders by a non-existent staffId returns 200 OK
         * with an empty list.
         *
         * @throws Exception if the MockMvc request fails
         */
    ...
    /**
     * Tests for PUT /api/v1/orders/{id}/status — order status update scenarios
     * including happy path, missing order, null status, and invalid transitions.
     */
    @Nested
    class UpdateOrderStatusTests {
        /**
         * Verifies that updating an existing order's status to IN_PROGRESS returns 200 OK,
         * and that a subsequent GET confirms the status has been persisted.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test         
        void updateOrderStatus_returns200() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/orders/{id}", orderId))
                    ...
                    .andExpect(jsonPath("$.customerId").value(customer.getId()))
                    .andExpect(jsonPath("$.stationId").value(station.getId()))
                    .andExpect(jsonPath("$.staffId").value(staff.getStaffId()))
                    .andExpect(jsonPath("$.orderStatus").value("IN_PROGRESS"));
        }

        /**
         * Verifies that updating the status of a non-existent order returns 404 Not Found,
         * as the service throws EntityNotFoundException when the order cannot be found.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test 
        ...
        /**
         * Verifies that updating an order's status with a null value returns 400 Bad Request,
         * as the @NotNull constraint on UpdateOrderStatusDto.orderStatus rejects null values.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test 
        ...
        /**
         * Verifies that transitioning a CANCELLED order to COMPLETED returns 409 Conflict,
         * as the service throws InvalidOrderStatusTransition when attempting
         * to change the status of an already cancelled order.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test
        ...
        /**
         * Verifies that updating the status of a non-existent staff returns 404 Not Found,
         * as the service throws EntityNotFoundException when the staff preparing the order cannot be found.
         *
         * @throws Exception if the MockMvc request fails
         */
        @Test
    ...
    /**
     * Tests for the automatic order cancellation scheduler — scenarios where
     * completed orders are cancelled after exceeding the 15-minute pickup window,
     * and where recent orders are left untouched.
     */
    @Nested 
    class AutoCancelationTests {
        ...
        void cancelUncollectedOrders_cancelsOrdersPastPickUpTime() throws Exception {
            ...
            PurchaseOrder updated = orderRepository.findById(order.getPurchaseOrderId()).orElseThrow();
    ...
    private Station saveTestStation() {
        Station station = stationRepository.save(new Station("Cramlington", "08:00-18:00", "09:00-17:00", true));
        return station;
    }
