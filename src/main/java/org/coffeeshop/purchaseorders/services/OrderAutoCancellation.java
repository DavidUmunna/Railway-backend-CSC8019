...
/**
 * Scheduled component that automatically cancels completed purchase orders
 * which have not been collected within 15 minutes of their requested pickup time.
 *
 * <p>Runs every 60 seconds via Spring's {@code @Scheduled} mechanism.
 * Queries the database for completed orders whose pickup time plus the
 * 15-minute grace period has elapsed, and sets their status to
 * {@link OrderStatus#CANCELLED}.</p>
 *
...
public class OrderAutoCancellation {
     ...
     */
    public OrderAutoCancellation(PurchaseOrderRepository orderRepository, PurchaseOrderService service) {
     ...
     *
     * <p>Calculates the cutoff as the current time minus 15 minutes, queries
     * for completed orders with a pickup time before that cutoff, and updates
     * each to {@link OrderStatus#CANCELLED}.</p>
     */
    ...
    public void cancelUncollectedOrders() {
        ...
        for (PurchaseOrder order : expired) {
            service.cancelOrder(order.getPurchaseOrderId());
        }
    }
