...
public class CustomerController {
     ...
     */
    @PostMapping("/create")
    public CompletableFuture<ResponseEntity<CustomerDto>> createCustomer(@Valid @RequestBody CreateCustomerDto dto) {
        CompletableFuture<CustomerDto> created = customerService.createCustomer(dto);
        return created.thenApply(saved->ResponseEntity.status(201).body(saved));
    }
